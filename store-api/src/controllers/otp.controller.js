...
// @desc    Gửi mã OTP để thay đổi email
// @route   POST /api/otp/change-email/:id
...
const sendOTPToChangeEmail = async (req, res, next) => {
  try {
    ...
    const templatePath = path.join(process.cwd(), 'src/templates/otpConfirmation.hbs')
...
// @desc    Xác minh mã OTP để thay đổi email
// @route   PATCH /api/otp/verify-email/:id
// @access  Private (admin, staff, member)
