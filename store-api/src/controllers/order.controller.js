...
// @desc    Tạo đơn hàng khi thanh toán
...
const createOrderCheckout = async (req, res, next) => {
  try {
    ...
    const customerTemplatePath = path.join(process.cwd(), 'src/templates/orderConfirmation.hbs')
    ...
    const adminTemplatePath = path.join(process.cwd(), 'src/templates/adminOrderNotification.hbs')
...
// @desc    Tạo thanh toán bằng MoMo
// @route   POST /api/orders/pay-with-momo/:userId
// @access  Private (member)
...
// @desc    Hoàn tất thanh toán MoMo (callback từ MoMo)
// @route   POST /api/orders/complete-pay-with-momo
// @access  Public
...
// @desc    Nhận trạng thái giao dịch từ MoMo (IPN)
// @route   POST /api/orders/transaction-status-momo
// @access  Public
...
// @desc    Tạo thanh toán bằng ZaloPay
// @route   POST /api/orders/pay-with-zalopay/:userId
// @access  Private (member)
const createPaymentWithZaloPay = async (req, res) => {
  ...
  const order = {
    ...
    amount: total_price,
    description: `Laptop Ease - Payment for the order #${transID}`,
    bank_code: '',
    callback_url:
      process.env.BUILD_MODE === 'prod'
...
// @desc    Hoàn tất thanh toán ZaloPay (callback từ ZaloPay)
// @route   POST /api/orders/complete-pay-with-zalopay
// @access  Public
...
// @desc    Nhận trạng thái giao dịch từ ZaloPay (IPN)
// @route   POST /api/orders/transaction-status-zalopay
// @access  Public
...
// @desc    Lấy đơn hàng theo ID đơn hàng
...
// @access  Private (member hoặc admin/staff nếu cho phép)
...
// @route   PATCH /api/orders/:userId/:orderId
// @access  Private (admin, member, staff)
