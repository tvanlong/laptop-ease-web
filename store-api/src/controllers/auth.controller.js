...
const signUp = async (req, res, next) => {
  try {
    ...
    const templatePath = path.join(process.cwd(), 'src/templates/accountConfirmation.hbs')
...
// @desc    Xác nhận đăng nhập thành công (callback)
// @route   POST /api/auth/signin-success
// @access  Public
...
const forgotPassword = async (req, res, next) => {
  try {
    ...
    const templatePath = path.join(process.cwd(), 'src/templates/newPassword.hbs')
    const source = fs.readFileSync(templatePath, 'utf8')
    const template = handlebars.compile(source)
